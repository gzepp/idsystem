...
import ArchivedStaffList from "./admin_data/staff_archivesList";

//functions
import { validateUsers, updateStaff, useAppUniidContext } from "../../context";
import { fetchStaff, decrypt } from "../../utils";

export default function AdminArchiveStaffs() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedStaffData = await fetchStaff({ getUserlist: "archived" }); // Fetch the data
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Archived Staff Records");
    ...
    // Save As
    saveAs(blob, "archived_staff_records.xlsx");
  };
            ...
            <div className="staff-rec-table-header-container">
              <div className="left-header-rec">
                <span className="staff-rec-title">
                  ...
                  <span className="staff-rec-text">Staff Archives</span>
                </span>
              </div>
